...
public class BubbleSortJava {
    ...
    private static final String OUTPUT_CSV = "resultados_bubblesort_java.csv";
    
    private static Random random = new Random(RANDOM_SEED);
    
    // Bubble Sort Serial
    public static int[] bubblesortSerial(int[] arr) {
        if (arr.length <= 1) {
            return arr;
        }
    ...
    // Bubble Sort para segmento
    private static int[] bubblesortSegment(int[] arrSegment) {
    ...
    // Função merge para segmentos ordenados
    ...
    public static int[] bubblesortParallel(int[] arr, int maxDepth) throws Exception {
        if (arr.length <= 1) {
            return arr;
        }
        ...
        if (maxDepth <= 0) {
            ...
            for (int[] seg : segments) {
                sortedSegments.add(bubblesortSegment(seg));
            }
        ...
        try {
            ...
            for (int[] seg : segments) {
                futures.add(executor.submit(() -> bubblesortSegment(seg)));
            }
    ...
    public static void benchmarkBubblesort() {
        ...
        System.out.println("Executando benchmark BubbleSort (Java)...\n");
                ...
                for (String dataType : DATA_TYPES) {
                    ...
                    for (int depth : THREAD_LEVELS) {
                        ...
                        for (int sample = 1; sample <= SAMPLES; sample++) {
                            ...
                            String mode;
                            if (depth == 0) {
                                bubblesortSerial(dataCopy);
                                mode = "Serial";
                            } else {
                                bubblesortParallel(dataCopy, depth);
                                mode = "Paralela";
                            }
                            ...
                            String[] row = {
                                "BubbleSort", mode, String.valueOf(size), dataType,
